# Phase 21 Security Acceptance: GA Release
...
- Fail-secure forwarding control: GA cannot ship unless fail-secure, no-PII logging, SBOM, vulnerability, property-test, SLO, tenant-isolation, and documentation gates are green.
...
- `anonreq_release_gate_status` must be exposed and covered by tests.
- `anonreq_migration_duration_seconds` must be exposed and covered by tests.
- `anonreq_ga_validation_tests_total` must be exposed and covered by tests.
- `anonreq_signed_artifacts_total` must be exposed and covered by tests.

## Required Audit Events

- release_candidate_built audit event must be emitted with metadata only.
- release_gate_passed audit event must be emitted with metadata only.
- release_gate_failed audit event must be emitted with metadata only.
- ga_release_approved audit event must be emitted with metadata only.
- artifact_signed audit event must be emitted with metadata only.
...
Go:
- All tests in `21-TEST-PLAN.md` pass.
