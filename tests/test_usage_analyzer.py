...
class TestUsageAnalyzer:
    ...
    def test_analyze_dns_groups_by_provider(self):
        ...
        entries = [
            DNSEntry("api.openai.com", "10.0.0.1", datetime(2026, 6, 20, 10, 0, 0, tzinfo=UTC)),
            DNSEntry("api.anthropic.com", "10.0.0.1", datetime(2026, 6, 20, 10, 1, 0, tzinfo=UTC)),
            DNSEntry("api.openai.com", "10.0.0.2", datetime(2026, 6, 20, 10, 2, 0, tzinfo=UTC)),
        ]
    ...
    def test_analyze_dns_deduplicates_users(self):
        ...
        entries = [
            DNSEntry("api.openai.com", "10.0.0.1", datetime(2026, 6, 20, 10, 0, 0, tzinfo=UTC)),
            DNSEntry("api.openai.com", "10.0.0.1", datetime(2026, 6, 20, 10, 1, 0, tzinfo=UTC)),
            DNSEntry("api.openai.com", "10.0.0.2", datetime(2026, 6, 20, 10, 2, 0, tzinfo=UTC)),
        ]
        result = self.analyzer.analyze_dns(entries, self.matcher)
        assert result["openai"].user_count == 2
    ...
    def test_analyze_dns_tracks_time_range(self):
        ...
        entries = [
            DNSEntry("api.openai.com", "10.0.0.1", datetime(2026, 6, 20, 10, 0, 0, tzinfo=UTC)),
            DNSEntry("api.openai.com", "10.0.0.1", datetime(2026, 6, 20, 12, 0, 0, tzinfo=UTC)),
        ]
        result = self.analyzer.analyze_dns(entries, self.matcher)
        assert result["openai"].first_seen == datetime(2026, 6, 20, 10, 0, 0, tzinfo=UTC)
        assert result["openai"].last_seen == datetime(2026, 6, 20, 12, 0, 0, tzinfo=UTC)

    def test_analyze_proxy_groups_by_provider(self):
        """analyze_proxy groups proxy entries by matched provider."""
        entries = [
            ProxyEntry("10.0.0.1", datetime(2026, 6, 20, 10, 0, 0, tzinfo=UTC), "POST",
                       "https://api.openai.com/v1/chat/completions", 200, 1500),
            ProxyEntry("10.0.0.1", datetime(2026, 6, 20, 10, 1, 0, tzinfo=UTC), "POST",
