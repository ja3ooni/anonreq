...
- Authenticated tenant-scoped requests exercise all OpenAPI endpoints: GET /v1/system/version, GET /v1/system/readiness, GET /v1/admin/release/evidence, GET /health, GET /metrics.
...
- customer pilot scenario suite.
- security acceptance gate aggregation.
...
- Required audit events are emitted: release_candidate_built, release_gate_passed, release_gate_failed, ga_release_approved, artifact_signed.
- Required metrics are present: anonreq_release_gate_status, anonreq_migration_duration_seconds, anonreq_ga_validation_tests_total, anonreq_signed_artifacts_total.
- OpenAPI schema validates and SDK contract tests remain green.
- Security acceptance gates in `21-SECURITY-ACCEPTANCE.md` pass without exception.
