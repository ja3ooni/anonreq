# Phase 21 Architecture: GA Release
...
- **ReleaseOrchestrator**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **MigrationRunner**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **ArtifactSigner**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **ReleaseNotesBuilder**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **SupportRunbookPack**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **CustomerValidationSuite**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **GAReadinessBoard**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
...
sequenceDiagram
  ...
  participant Phase as Phase 21 Components
  ...
  Gateway->>Phase: Execute GA Release decision/workflow
...
GA cannot ship unless fail-secure, no-PII logging, SBOM, vulnerability, property-test, SLO, tenant-isolation, and documentation gates are green. Additional threats are tenant confusion, stale configuration, unauthorized administration, log injection, schema drift, and denial-of-service through expensive evaluation paths. Mitigations are context-derived tenant scoping, immutable config versioning, RBAC checks, field allowlists, OpenAPI contract tests, latency budgets, and bounded queues.

## OpenAPI Changes

- `GET /v1/system/version`
- `GET /v1/system/readiness`
- `GET /v1/admin/release/evidence`
- `GET /health`
- `GET /metrics`
...
- `anonreq_release_gate_status`
- `anonreq_migration_duration_seconds`
- `anonreq_ga_validation_tests_total`
- `anonreq_signed_artifacts_total`
