# Phase 19 Test Plan: SDK Ecosystem
...
- Authenticated tenant-scoped requests exercise all OpenAPI endpoints: GET /openapi.json, GET /v1/models, POST /v1/chat/completions, GET /v1/transparency/{session_id}.
...
- SDK generated client compilation.
- round-trip examples for every language.
- streaming split-token examples.
- auth failure ergonomics.
...
- Required audit events are emitted: sdk_contract_release, openapi_schema_published, example_round_trip_verified.
- Required metrics are present: anonreq_sdk_contract_tests_total, anonreq_openapi_schema_hash, anonreq_client_errors_total.
- OpenAPI schema validates and SDK contract tests remain green.
- Security acceptance gates in `19-SECURITY-ACCEPTANCE.md` pass without exception.
