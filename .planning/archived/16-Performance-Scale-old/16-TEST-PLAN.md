# Phase 16 Test Plan: Performance and Scale
...
- Authenticated tenant-scoped requests exercise all OpenAPI endpoints: GET /v1/governance/status, GET /v1/admin/performance/profile, GET /metrics.
...
- P95 100ms prompt overhead.
- transparent proxy 5ms policy-only overhead.
- audio P99 150ms latency.
- 100 concurrent disconnect cleanup.
...
- Required audit events are emitted: slo_breach_detected, load_shed_event, capacity_profile_generated, performance_gate_failed.
- Required metrics are present: anonreq_processing_overhead_seconds, anonreq_stream_tail_buffer_flushes_total, anonreq_backpressure_rejections_total, anonreq_audio_pipeline_latency_seconds.
- OpenAPI schema validates and SDK contract tests remain green.
- Security acceptance gates in `16-SECURITY-ACCEPTANCE.md` pass without exception.
