# Phase 16 Architecture: Performance and Scale
...
- **LoadShedder**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **BackpressureController**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **LatencyBudgetManager**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **StreamingRestorationProfiler**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **ValkeyConnectionPoolTuner**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **K6ScenarioSuite**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **CapacityModel**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
...
sequenceDiagram
  ...
  participant Phase as Phase 16 Components
  ...
  Gateway->>Phase: Execute Performance and Scale decision/workflow
...
Performance optimizations cannot bypass detection, tokenization, policy evaluation, response restoration, or audit emission; overload returns controlled 429/503 responses. Additional threats are tenant confusion, stale configuration, unauthorized administration, log injection, schema drift, and denial-of-service through expensive evaluation paths. Mitigations are context-derived tenant scoping, immutable config versioning, RBAC checks, field allowlists, OpenAPI contract tests, latency budgets, and bounded queues.

## OpenAPI Changes

- `GET /v1/governance/status`
- `GET /v1/admin/performance/profile`
- `GET /metrics`
...
- `anonreq_processing_overhead_seconds`
- `anonreq_stream_tail_buffer_flushes_total`
- `anonreq_backpressure_rejections_total`
- `anonreq_audio_pipeline_latency_seconds`
