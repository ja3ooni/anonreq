# Phase 20 Architecture: SOC2 and ISO Readiness
...
- **ControlCatalog**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **EvidenceCollector**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **PolicyDocumentSet**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **TraceabilityMatrix**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **RiskRegisterWorkflow**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **AuditReadinessExporter**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **VendorQuestionnaireResponder**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
...
sequenceDiagram
  ...
  participant Phase as Phase 20 Components
  ...
  Gateway->>Phase: Execute SOC2 and ISO Readiness decision/workflow
...
Readiness evidence is generated from immutable records and signed artifacts; manual uploads are hash-recorded and cannot overwrite historical evidence. Additional threats are tenant confusion, stale configuration, unauthorized administration, log injection, schema drift, and denial-of-service through expensive evaluation paths. Mitigations are context-derived tenant scoping, immutable config versioning, RBAC checks, field allowlists, OpenAPI contract tests, latency budgets, and bounded queues.

## OpenAPI Changes

- `GET /v1/admin/compliance/report`
- `GET /v1/admin/governance/export`
- `GET /v1/admin/risk-assessments/export`
...
- `anonreq_controls_implemented_total`
- `anonreq_evidence_freshness_seconds`
- `anonreq_risk_treatments_overdue_total`
