# Phase 12 Architecture: DLP and Advanced Detection
...
- **DLPClassifier**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **AdvancedRecognizerRegistry**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **PromptSecurityEngine**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **OutputPolicyScanner**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **MNPIRecognizer**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **FinancialCrimeRecognizer**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
- **FairnessDatasetRunner**: owns a bounded part of the phase capability and exposes typed interfaces to the gateway orchestration layer.
...
sequenceDiagram
  ...
  participant Phase as Phase 12 Components
  ...
  Gateway->>Phase: Execute DLP and Advanced Detection decision/workflow
...
DLP action resolution is deny-biased: block outranks quarantine, quarantine outranks redact, redact outranks anonymize, and anonymize outranks allow. Additional threats are tenant confusion, stale configuration, unauthorized administration, log injection, schema drift, and denial-of-service through expensive evaluation paths. Mitigations are context-derived tenant scoping, immutable config versioning, RBAC checks, field allowlists, OpenAPI contract tests, latency budgets, and bounded queues.

## OpenAPI Changes

- `GET /v1/admin/dlp/policies`
- `PUT /v1/admin/dlp/policies/{policy_id}`
...
- `anonreq_firewall_events_total`
- `anonreq_detection_quality_score`
