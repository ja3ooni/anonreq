# Phase 09 Security Acceptance: RBAC and SSO
...
- Fail-secure forwarding control: Administrative endpoints deny access unless a verified principal has a role granting the exact endpoint permission; external IdP outages fail closed.
...
- `anonreq_auth_events_total` must be exposed and covered by tests.
- `anonreq_authz_denials_total` must be exposed and covered by tests.
- `anonreq_jwks_refresh_total` must be exposed and covered by tests.
- `anonreq_revocation_lookup_seconds` must be exposed and covered by tests.

## Required Audit Events

- auth_success audit event must be emitted with metadata only.
- auth_failure audit event must be emitted with metadata only.
- authz_denied audit event must be emitted with metadata only.
- api_key_created audit event must be emitted with metadata only.
- api_key_revoked audit event must be emitted with metadata only.
- sso_mapping_updated audit event must be emitted with metadata only.
...
Go:
- All tests in `09-TEST-PLAN.md` pass.
