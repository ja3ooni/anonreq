# Phase 09 Test Plan: RBAC and SSO
...
- Authenticated tenant-scoped requests exercise all OpenAPI endpoints: GET /v1/security/status, POST /v1/admin/api-keys, DELETE /v1/admin/api-keys/{key_id}, GET /v1/admin/auth/events.
...
- JWT signature, expiry, audience, issuer validation.
- SAML assertion replay rejection.
- mTLS CN/SAN allowlist enforcement.
- role matrix coverage.
- revocation cache outage behavior.
...
- Required audit events are emitted: auth_success, auth_failure, authz_denied, api_key_created, api_key_revoked, sso_mapping_updated.
- Required metrics are present: anonreq_auth_events_total, anonreq_authz_denials_total, anonreq_jwks_refresh_total, anonreq_revocation_lookup_seconds.
- OpenAPI schema validates and SDK contract tests remain green.
- Security acceptance gates in `09-SECURITY-ACCEPTANCE.md` pass without exception.
